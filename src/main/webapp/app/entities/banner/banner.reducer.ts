...
const apiSearchUrl = 'api/_search/banners';

// Actions

export const getSearchEntities: ICrudSearchAction<IBanner> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_BANNERS,
  payload: axios.get<IBanner>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
